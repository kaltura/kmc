...
//			Custom
			...
			_dateRange = [resourceManager.getString('analytics', 'curYear'),
				resourceManager.getString('analytics', 'curMonth'),
				resourceManager.getString('analytics', 'curPrevMonth'),
				resourceManager.getString('analytics', 'prevMonth'),
				resourceManager.getString('analytics', 'curQtr'),
				resourceManager.getString('analytics', 'curPrevQtr'),
				resourceManager.getString('analytics', 'prevQtr'),
				resourceManager.getString('analytics', 'curPrevYear'),
				resourceManager.getString('analytics', 'prevYear'),
		...
		public function changeRangeByDate(filterVo:FilterVo):int {
			...
			if (filterVo.fromDate.fullYear == today.fullYear 
				&& filterVo.fromDate.month == 0 && filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month && filterVo.toDate.date == today.date) {
				
				result = resourceManager.getString('analytics', 'curYear');
			}
			else if (filterVo.fromDate.fullYear == today.fullYear 
				&& filterVo.fromDate.month == today.month
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month
				&& filterVo.toDate.date == today.date) {
				
				result = resourceManager.getString('analytics', 'curMonth');
			}
			else if (filterVo.fromDate.fullYear == today.fullYear 
				&& filterVo.fromDate.month == today.month - 1
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month
				&& filterVo.toDate.date == today.date) {
				...
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month-1
				&& filterVo.toDate.date == getLastDayOfMonth(today.month-1)) {
				
				result = resourceManager.getString('analytics', 'prevMonth');
			}
			else if (filterVo.fromDate.fullYear == today.fullYear 
				&& filterVo.fromDate.month == getFirstMonthOfQtr(today.month)
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month
				&& filterVo.toDate.date == today.date) {
				...
				&& filterVo.fromDate.month == getFirstMonthOfQtr(today.month-3)
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month
				&& filterVo.toDate.date == today.date) {
				...
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == getLastMonthOfQtr(today.month-3)
				&& filterVo.toDate.date == getLastDayOfMonth(filterVo.toDate.month)) {
				
				result = resourceManager.getString('analytics', 'prevQtr');
			}
			else if (filterVo.fromDate.fullYear == today.fullYear-1 
				&& filterVo.fromDate.month == 0
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear
				&& filterVo.toDate.month == today.month
				&& filterVo.toDate.date == today.date) {
				...
				&& filterVo.fromDate.month == 0
				&& filterVo.fromDate.date == 1
				&& filterVo.toDate.fullYear == today.fullYear-1
				&& filterVo.toDate.month == 11
				&& filterVo.toDate.date == 31) {
